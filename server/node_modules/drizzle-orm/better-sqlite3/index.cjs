...
_a = index.entityKind;
BetterSQLiteSession[_a] = 'BetterSQLiteSession';
class BetterSQLiteTransaction extends session.SQLiteTransaction {
    transaction(transaction) {
        ...
        this.session.run(index.sql.raw(`savepoint ${savepointName}`));
        try {
            const result = transaction(tx);
            this.session.run(index.sql.raw(`release savepoint ${savepointName}`));
            return result;
        }
        catch (err) {
            this.session.run(index.sql.raw(`rollback to savepoint ${savepointName}`));
            throw err;
        }
    }
}
_b = index.entityKind;
BetterSQLiteTransaction[_b] = 'BetterSQLiteTransaction';
class PreparedQuery extends session.PreparedQuery {
    ...
    get(placeholderValues) {
        ...
        return index.mapResultRow(fields, row, joinsNotNullableMap);
    }
...
_c = index.entityKind;
PreparedQuery[_c] = 'BetterSQLitePreparedQuery';
