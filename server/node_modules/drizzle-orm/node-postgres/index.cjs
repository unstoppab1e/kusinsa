...
var _a$1, _b, _c;
const { Pool } = pg;
class NodePgPreparedQuery extends index.PreparedQuery {
    ...
    async execute(placeholderValues = {}) {
        return index.tracer.startActiveSpan('drizzle.execute', async () => {
            ...
            if (!fields && !customResultMapper) {
                return index.tracer.startActiveSpan('drizzle.driver.execute', async (span) => {
                    span?.setAttributes({
            ...
            const result = await index.tracer.startActiveSpan('drizzle.driver.execute', (span) => {
                span?.setAttributes({
    ...
    all(placeholderValues = {}) {
        return index.tracer.startActiveSpan('drizzle.execute', () => {
            const params = index.fillPlaceholders(this.params, placeholderValues);
            this.logger.logQuery(this.rawQuery.text, params);
            return index.tracer.startActiveSpan('drizzle.driver.execute', (span) => {
                span?.setAttributes({
...
class NodePgSession extends index.PgSession {
...
class NodePgTransaction extends index.PgTransaction {
